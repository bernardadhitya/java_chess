...
public class Rook extends Piece{
	int toAttack = -1;
	public Rook(int x, int y, char type) {
		super(x, y, type);
	...
	public boolean isAvailable(int dest_x, int dest_y, Vector<Piece> team, Vector<Piece> enemy) {
		...
		if (Character.isLowerCase(this.getType())) {
			...
			System.out.println("Rook of the White ");
			if ((dest_y == this.getY() || dest_x == this.getX()) && checkMove(dest_x,dest_y,team,enemy)) {
				if(toAttack != -1) {
			...
			System.out.println("Rook of the Black ");
			if ((dest_y == this.getY() || dest_x == this.getX()) && checkMove(dest_x,dest_y,team,enemy)) {
				if(toAttack != -1) {
					System.out.println("Proceed to attack...");
					this.setX(dest_x);
